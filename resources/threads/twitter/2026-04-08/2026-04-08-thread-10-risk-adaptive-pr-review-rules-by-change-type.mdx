...
title: Thread 10 - risk adaptive pr review rules by change type
description: >-
  Advanced Kody tip: don’t apply the same review logic to every PR. A small fix,
  a large refactor, and a breaking change need different checks. With PR
  variables, you can make Kody Rules adapt to the PR context, like size, labels,
  description, and number of files changed. Create a Pull Request rule, click
  Variables, and use them directly in the instructions.
