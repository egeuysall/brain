...
function whyFit(startup) {
  const points = [];
  if (startup.userCategory) points.push(`${startup.userCategory.toLowerCase()} company`);
  if ((startup.tags || []).includes("developer-tools")) points.push("developer-tools tag");
  if ((startup.tags || []).includes("security")) points.push("security tag");
  if ((startup.tags || []).includes("analytics")) points.push("analytics tag");
  if ((startup.tags || []).includes("customer-support")) points.push("customer-support tag");
  if (startup.businessType === "B2B") points.push("B2B");
  if (startup.founderTwitter) points.push(`founder on X (${startup.founderTwitter})`);
  if ((startup.founderFollowers || 0) >= 200 && (startup.founderFollowers || 0) <= 15000) {
    points.push("reply-sized audience");
  }
  if ((startup.mrr || 0) >= 2000) points.push(`real revenue (${formatMoney(startup.mrr)} MRR)`);
  return points.slice(0, 5).join(", ");
}

function caveat(startup) {
  const caveats = ["dataset lacks role", "dataset lacks team size", "dataset lacks recent trigger"];
...
lines.push("- Boosted B2B, software, devtools/security/analytics/support signals, founder X presence, moderate follower counts, and non-trivial MRR.");
lines.push("- Penalized agency/creator/SEO/social-heavy entries and sale listings.");
lines.push("- Important: dataset does **not** include role, team size, or recent founder trigger. This file is best used as priority-enrich list, not final DM list.");
lines.push("");
lines.push("## Ranked table");
lines.push("");
lines.push("| # | Startup | Founder | Category | MRR | X followers | score | why it surfaced |");
lines.push("|---|---|---|---|---:|---:|---:|---|");
...
shortlist.forEach((startup, index) => {
  ...
  lines.push(`- Founder: ${startup.founderName || "Unknown"}${startup.founderTwitter ? ` ([x.com/${startup.founderTwitter}](https://x.com/${startup.founderTwitter}))` : ""}`);
  lines.push(`- Why fit: ${whyFit(startup)}.`);
  lines.push(`- Problem: ${startup.problemSolved || startup.description || "No problem statement in dataset."}`);
  lines.push(`- Target: ${startup.targetPersona || "No target persona in dataset."}`);
  lines.push(`- Caveat: ${caveat(startup)}.`);
  lines.push(`- Links: [TrustMRR](${startup.url})${startup.website ? `, [Website](${startup.website})` : ""}`);
  lines.push("");
});
