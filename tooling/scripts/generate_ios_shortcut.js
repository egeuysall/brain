...
const OUTPUT_DIR =
  process.env.OUTPUT_DIR || path.resolve(process.cwd(), "tooling", "shortcuts")
    ...
    WFJSONValues: {
      Value: {
        WFDictionaryFieldValueItems: [
          dictStringItem(
            "message",
            withVariables`chore(resources): add ${stampOut}.md`
          ),
...
const actions = [
  ...
  text(
    { text: withVariables`https://r.jina.ai/${sourceUrlOut}` },
    readerUrlOut
  ),
  ...
  text(
    {
      text: withVariables(
        [
          `https://api.github.com/repos/${OWNER}/${REPO}/contents/resources/`,
          ".md"
        ],
        stampOut
      )
